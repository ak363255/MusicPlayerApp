...
class MusicAdapter(private val callback:(Song)->Unit):RecyclerView.Adapter<MusicAdapter.MusicViewHolder>() {
    ...
    private fun initUi(binding: MusicItemViewBinding, position: Int) {
        ...
        binding.duration.setTextColor(ContextCompat.getColor(binding.root.context, R.color.white))
        binding.albumName.setTextColor(ContextCompat.getColor(binding.root.context, R.color.white))
